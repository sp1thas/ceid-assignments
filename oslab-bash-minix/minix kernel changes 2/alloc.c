  ...
  phys_clicks clicks; /* amount of memory requested */
{
   ...
   */
  register struct hole *hp, *prev_ptr, *min_slot, *prev_min_slot;	
  phys_clicks old_base;

/*dhmiourgoume dyo neous pointer pou deixnoun:
 *	o min_slot thn katallhlh oph me vash th stratigikh best fit thn opoia  
 *tha xrhsimopoihsoume gia thn anathesh mnhmhs sth diergasia
 * 	o prev_min_slot deixnei thn prohgoumenh oph apo afth pou epileksame    
 *wste se periptwsh pou h diergasia exei idio megethos me to slot tote na 
 *to diagrapsoume kai na kseroume poio einai to prohgoumeno slot
 */
...
/*me afth th while epilegoume to katallhlo slot gia thn anathesh*/
    {
      if (hp->h_len >= clicks && min_slot->h_len > hp->h_len)	
	/*pera apo to an to slot exei megethos >= apo thn diergasia elegxoyme 		 
	 *an to slot einai > apo afto pou mexri stigmhs einai to mikrotero 		 
	 *dynato ( min_slot) an einai tote proxwrame sto epomeno
	 */
      {
		min_slot=hp;				/* vrethike slot gia to opoio menei*/ 								/*ligoteros kenos xwros an ginei*/ 			 			prev_min_slot = prev_ptr;		/*anathesh ths diergasias kai giafto*/ 								/*allazoume tis times stous dyo*/ 								/*pointer wste na deixnoyn to neo 								 *slot kai to prohgoumeno toy*/
	...
	/*afou vrethike to katallhlo slot to opoio deixnei o pointer min_slot
	 *proxwrame sthn anathesh ths mnhmhs tou sthn diergasia*/

	/*profanws h anathesh tha ginei me thn xrhsh tou pointer min_slot o 	 
	 *opoios deixnei thn katallhlh oph kai oxi me ton pointer hp*/
	
        /* We found a hole that is big enough.  Use it. */
        old_base = min_slot->h_base; /* remember where it started */
        min_slot->h_base += clicks; /* bite a piece off */
        min_slot->h_len -= clicks; /* ditto */

        /* Delete the hole if used up completely. */
        if (min_slot->h_len == 0)	
	/*an to slot gemisei tote to diagrafoume me th xrhsh ths synarthshs 	 
	 *del_slot me orismata to slot to opoio xrhsimopoihsame kai me to 		 
	 *prohgoumeno to opoio to deixnei o pointer prev_min_slot pou 	
	 *ftiaksame*/ 

          del_slot(prev_min_slot, min_slot);
